...
type Message struct {
	...
	RecipientId *int         `db:"recipientId" json:"recipientId"`
...
func FindAllMessage(limit int, offset int) (InfoMessage, error) {
	sql := `SELECT * FROM "message" 
	ORDER BY "id" ASC
	LIMIT $1
	OFFSET $2`
	sqlCount := `SELECT COUNT(*) FROM "message"`
	result := InfoMessage{}
	data := []Message{}
...
func CreateMessage(data Message) (Message, error) {
	...
	VALUES (:recipientId, :senderId, :text)
	RETURNING *`
